...
GenieTouchBar.prototype.Draw = function() {
   ...
   if (this.Specs.COLOUR)
      this.Context.fillStyle = this.Specs.COLOUR.KEY;
   else
...
GenieTouchBar.prototype.ClickedOn = function() {
   this.i = this.GetKey();
...
GenieTouchBar.prototype.GetKey = function() {
   if (this.Specs.ORIENT==ORIENTATION.HORIZONTAL)
      return (Math.floor((Mouse.Click.X-this.Specs.L)/(this.Specs.KEY.W+1)));
   else
      return (Math.floor((Mouse.Click.Y-this.Specs.T)/(this.Specs.KEY.H+1)));
};
