 ...
 */
//------------------------------------------------
//---------- GENIE ICON PANEL --------------------  specs = { L: -1, T: -1, W: -1, H: -1, R: 2, C: 4, ICONS: -1, ICON: { W: -1, H: -1 }, PRESS: -1, COLOURS: "" };
...
GenieIconPanel.prototype.Draw = function() {
	...
	this.GraphicsTool.SwitchContext(this.Context);
	for (indx=0;indx<this.Specs.ICONS;++indx)
		if (indx==this.DepressedIcon)
	 this.DrawIcon(indx, PRESSED);
		else
	 this.DrawIcon(indx);
	this.GraphicsTool.RestoreContext();
};
GenieIconPanel.prototype.GetIconPressed = function() {  //returns index
	...
	this.x = Math.floor((Mouse.Down.X-this.Specs.L)/(this.Specs.ICON.W+6));
	this.y = Math.floor((Mouse.Down.Y-this.Specs.T)/(this.Specs.ICON.H+6));
...
GenieIconPanel.prototype.DrawIcon = function(iIcon, bPressed) {
	...
	this.x = this.Specs.L + Math.round((this.Specs.ICON.W+7)*(iIcon % this.Specs.C));
	this.y = this.Specs.T + Math.round((this.Specs.ICON.H+7)*Math.floor(iIcon/this.Specs.C));

	//Draw shadows
	this.GraphicsTool.DrawRectangle(this.x+3, this.y, this.Specs.ICON.W, 3, colour1, 0);								//top shadow
	this.GraphicsTool.DrawRectangle(this.x+this.Specs.ICON.W+3, this.y+3, 3, this.Specs.ICON.H, colour2, 0);		//right shadow
	this.GraphicsTool.DrawRectangle(this.x+3, this.y+this.Specs.ICON.H+3, this.Specs.ICON.W, 3, colour2, 0);		//bottom shadow
	this.GraphicsTool.DrawRectangle(this.x, this.y+3, 3, this.Specs.ICON.H, colour1, 0);								//left shadow

	//Draw Image or colour
	if (this.Pic)
		this.Pic.DrawPatchNumber(iIcon, this.x+3, this.y+3);
	else
		this.GraphicsTool.DrawRectangle(this.x+3, this.y+3, this.Specs.ICON.W, this.Specs.ICON.H, this.Specs.COLOURS[iIcon], 0);

	//Draw corners
	this.CornersPic.DrawPatchNumber(num, this.x, this.y);			//top-left corner
	this.x += (this.Specs.ICON.W+3);
	this.CornersPic.DrawPatchNumber(num+1, this.x, this.y);		//top-right
	this.y += (this.Specs.ICON.H+3);
...
GenieIconPanel.prototype.EraseIcon = function(iIcon) {
	...
	//Determine correct coordinates
	this.x = this.Specs.L + ((this.Specs.ICON.W+7)*(iIcon % this.Specs.C));
	this.y = this.Specs.T + ((this.Specs.ICON.H+7)*Math.floor(iIcon/this.Specs.C));

	if (this.Specs.COLOUR) {
		this.Context.fillStyle = this.Specs.COLOUR;
		this.Context.fillRect(this.x, this.y, this.Specs.ICON.W+6, this.Specs.ICON.H+6);
	} else
		this.Context.clearRect(this.x, this.y, this.Specs.ICON.W+6, this.Specs.ICON.H+6);
};
GenieIconPanel.prototype.UnPress = function() {
	if (this.DepressedIcon==-1)
		return;
	this.x = this.Specs.L + ((this.Specs.ICON.W+7)*(this.DepressedIcon % this.Specs.C));
	this.y = this.Specs.T + ((this.Specs.ICON.H+7)*Math.floor(this.DepressedIcon/this.Specs.C));
	if (this.Specs.COLOUR) {
		this.Context.fillStyle = this.Specs.COLOUR;
		this.Context.fillRect(this.x, this.y, this.Specs.ICON.W+6, this.Specs.ICON.H+6);
	} else
