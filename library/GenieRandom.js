...
GenieRandomizer.prototype = {
	...
	GetNumber(num) {  //returns a value between 1 and num
	...
	CheckBoolean() {  //TODO: replace w/ ::FlipCoin
...
//				this.Value = start + Math.floor(((end-start)+1)*Math.random());  TODO: this will be REDUNDANT if there is no thrashing
	...
	Shuffle(aNum, bInit) {  //NOTE: this is Durstenfeld's version of the Fisher-Yates Shuffle, and more efficient than ::GetUniqueNumbers
	...
	Get2DElement(arry) {  //NOTE: this method should not be used in a loop to avoid GC
		var aSlots;

		aSlots = new Array(arry.length);
		for (this.i=0;this.i<arry.length;++this.i)
			aSlots[this.i] = arry[this.i].length;
		this.i = this.GetSlot(aSlots);
		this.j = this.GetIndex(arry[this.i].length);

		return (arry[this.i][this.j]);
	}
};
/* REDUNDANT - this what it used to be
	GenerateSeeds() {
